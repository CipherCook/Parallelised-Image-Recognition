...
float* neural_net_run(float *input, float* output, cudaStream_t &stream){
    // input size = 28*28
    // output size = 10
    ...
    conv1_kernel<<<gridSize, blockSize,0,stream>>>(d_input, d_output, d_conv1_weights, d_conv1_biases);
    // cout<<"Launched conv1 for stream "<<id<<'\n';
    // Print the matrix
    ...
    maxpool_1_kernel<<<gridSize1, blockSize1,0,stream>>>(d_output, d_output1);
    ...
    conv2_kernel<<<gridSize2, blockSize2,0,stream>>>(d_output1, d_output2, d_conv2_weights, d_conv2_biases);
    // cout<<"Launched conv2 for stream "<<id<<'\n';
    ...
    maxpool_2_kernel<<<gridSize3, blockSize3,0,stream>>>(d_output2, d_output3);
    ...
    fc_1_kernel<<<gridSize4, blockSize4,0,stream>>>(d_output3, d_output4, d_fc1_weights, d_fc1_biases);
    ...
    // The output size is 500
    RelU_fc1<<<1, 500,0,stream>>>(d_output4);
    // print_matrix<<<1,1>>>(d_output4,20,25);
    ...
    fc_2_kernel<<<gridSize5, blockSize5,0,stream>>>(d_output4, d_output5, d_fc2_weights, d_fc2_biases);
    ...
    softmax_1<<<1, 10,0,stream>>>(d_output5, d_output6, d_exp_total_sum, 10);
    softmax_2<<<1, 10,0,stream>>>(d_output6, d_exp_total_sum, 10);
...
void* thread_func(void* args){
    //args include : input vector, output vector, stream address
    thread_args* t_args = (thread_args*) args;
    neural_net_run(t_args->input, t_args->output, t_args->stream);
    return NULL;
}
...
int main(int argc, char* argv[]){
    ...
    thread_handles = new pthread_t[numStreams];
    FILE *f = fopen("/content/trained_weights/conv1.txt", "r");
    ...
    fclose(f);
    FILE *f1 = fopen("/content/trained_weights/conv2.txt", "r");
    ...
    fclose(f1);
    FILE *f2 = fopen("/content/trained_weights/fc1.txt", "r");
    ...
    fclose(f2);
    FILE *f3 = fopen("/content/trained_weights/fc2.txt", "r");
    ...
    float** input = new float*[numStreams];
    float** output = new float*[numStreams];
    for(int i=0;i<numStreams;i++){
        input[i] = new float[28*28];
        output[i] = new float[10];
    }
    cout<<"I am here "<<'\n';
    // Let's create numStreams streams
    cudaStream_t streams[numStreams];
    for(int i = 0; i < numStreams; i++){
        cudaStreamCreate(&streams[i]);
    }
    auto start = chrono::high_resolution_clock::now();
    for(int i=0; i<(1000/numStreams); i++){
        for(int j=0; j<numStreams;j++){
            string filename = "/content/tst_imgs/" + to_string(10005000+i*numStreams+j) + ".txt";
            FILE *f_a = fopen(filename.c_str(), "r");
            for(int k = 0; k < 28 * 28; k++){
                fscanf(f_a, "%f", &input[j][k]);
            }
            fclose(f_a);
            // launch a kernel with the ith stream
            //cout<<i<<' '<<j<<'\n';
            thread_args* args = new thread_args();
            args->input = input[j];
            args->output = output[j];
            args->stream = streams[j];
            pthread_create(&thread_handles[j], NULL, thread_func, (void*) args);
        }
        for(int j=0;j<numStreams;j++){
